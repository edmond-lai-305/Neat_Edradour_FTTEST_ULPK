    ...
    public partial class FormMain
    {
        ...
        bool check_result(string sn)
        {
            try
            {
                ...
                textBoxAck.AppendText(@"\\10.2.100.27\File Bak\oneline_data\NEATFRAME\TDM\PASS\" + "\r\n");
                //foreach (var file in Directory.GetFiles(@"\\192.168.158.26\cisco\NEATFRAME\TDM\PASS\", sn + "*.csv"))
                foreach (var file in Directory.GetFiles(@"\\10.2.100.27\File Bak\oneline_data\NEATFRAME\TDM\PASS\", sn + "*.csv"))
                {
   ...
   try {
     ...
     GetFw:
        ...
        bool Barra_GetTvSn2()
        {
            ...
            if (ack.Contains(textBoxSn.Text.Trim()) == false)
            {
                RunAdbCmd("adb devices", out ack);
            }
            else
            ...
            //RunAdbCmd("adb shell su root nf_persist | grep serialno", out ack);
            //if (ResultPick(ack, @"(?<=serialno=).*?(?=\r|\n)", out ack2) == false)
            SetMsg("sn:" + ack, UDF.COLOR.WORK);
            if (ack.Contains(textBoxSn.Text.Trim()) ==false )
            {
                SetMsg("Err:" + ack, UDF.COLOR.FAIL);
                SetMsg("机内sn与扫码标签sn不符，请检查!", UDF.COLOR.FAIL);
                this.Refresh();
                if (iRetryTime > 2)
                    return false;
                else
